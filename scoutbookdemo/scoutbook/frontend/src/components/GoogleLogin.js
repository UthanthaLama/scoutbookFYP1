...
export default function GoogleLogin({ onSuccess, onError }) {
  ...
  const handleGoogleLogin = async () => {
    if (!isConfigured) {
      onError('Google OAuth is not configured. Please set up Google credentials.');
      return;
    }
    ...
    return (
      <div className="google-oauth-notice">
        <p className="notice-text">
          ℹ️ Google Sign-In is not configured yet. 
          <br />
          <small>Administrator needs to set up Google OAuth credentials.</small>
        </p>
      </div>
    );
  }
